...
function findElementGreaterThan(arr, num) {
  ...
  const foundElem = arr.find((element) => element > num);
  if (foundElem !== undefined) {
    return foundElem;
  } else {
    return "Число, яке задовольняє умову, не знайдено";
  }
}
...
function findIndexByProperty(arr, name) {
  ...
  // Використовуємо метод findIndex для пошуку індексу об'єкта, властивість 'name'
  //якого співпадає з заданим іменем.
  // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
  const foundIndex = arr.findIndex((element) => element.name === name);
  // return arr.findIndex(({ name }) => name !== undefined);
...
// Завдання: 4
/**
 * Функція `getSubArrayAndConvertToString` отримує підмасив від заданого індексу до кінця
 *  масиву і конвертує його в рядок.
...
function getPositiveArrayValues(arr) {
  ...
  const FilterFn = (value, index, array) => value > 0;
  const objPositiveArrayValues = arr.filter(FilterFn);
  return objPositiveArrayValues;
}
...
function wordLengths(arr) {
  ...
  // Використовуємо метод map для створення нового масиву, де кожний елемент - це
  //довжина відповідного слова з вхідного масиву
